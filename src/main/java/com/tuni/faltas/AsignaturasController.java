...
public class AsignaturasController implements Initializable {
	...
	private void comboBoxCurso() {
		...
		if (asignaturas != null) {
			...
			listAsignatura = comboBoxCursos.getSelectionModel().getSelectedItem().getListAsignatura();
			comboBoxAsignaturas.setItems(comboBoxCursos.getSelectionModel().getSelectedItem().getListAsignatura());
		}
	}
